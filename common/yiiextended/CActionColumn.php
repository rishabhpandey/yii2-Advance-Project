 ...
 * @since 2.0
 */
class CActionColumn extends Column
{
    /**
    ...
    protected function initDefaultButtons()
    {
        ...
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                ...
                return Html::a('<i class="glyphicon glyphicon-edit"></i>', $url, $options);
            };
        }
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                
				if((isset($model->is_deletable) && $model->is_deletable=='1') || !(isset($model->is_deletable))){
					$options = array_merge([
					...
					$options = array_merge([
						'onclick' => 'javascript:alert("Non deletable record !!!!!");',
					], $this->buttonOptions);
					return Html::a('<i class="glyphicon glyphicon-trash text-muted"></i>', '#',$options);
				}
                            
            };
        }
		
        if (!isset($this->buttons['change-status'])) {
            $this->buttons['change-status'] = function ($url, $model, $key) {
				...
				if($model->is_active=='1')
					{
						$options = array_merge([
							...
							'data-confirm' => Yii::t('yii', 'Are you sure you want to deactivate this record?'),
							'data-method' => 'post',
							'data-pjax' => '1',
							'class' => 'text-red',
						], $this->buttonOptions);					
						return Html::a('<i class="glyphicon glyphicon-ban-circle"></i>', $url, $options);
					}else{
					
						$options = array_merge([
							'title' => Yii::t('yii', 'Activate'),
							'aria-label' => Yii::t('yii', 'Activate'),
							'data-confirm' => Yii::t('yii', 'Are you sure you want to activate this record?'),
							'data-method' => 'post',
							'data-pjax' => '1',
							'class' => 'text-success',
						], $this->buttonOptions);
						return Html::a('<i class="glyphicon glyphicon-ok-circle"></i>', $url, $options);
					}
				}
            };
        }
